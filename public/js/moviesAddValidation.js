window.onload = function () {
    ...
    form.addEventListener("submit", (e) => {
      ...
      if (rating.value <= 0 || rating.value > 10.0) {
        errors.push(
          "El campo calificación no puede ser menor a cero ni mayor a 10"
        );
      ...
      if (length.value < 60 || length.value > 360) {
        errors.push(
          "El campo duración no puede ser menor a 60 ni mayor a 360 minutos"
        );
      ...
      if (errors.length > 0) {
        ...
        for (let i = 0; i < errors.length; i++) {
          ulErrors.innerHTML += `<li >  ${errors[i]} </li>`; //"<li >" +  errors[i] + "</li>"
        }
      } else {
